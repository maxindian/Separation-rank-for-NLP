...
best_result = {
    "epoch": 0, "train_acc": 0, "test_f1": 0, "train_acc": 0, "valid_f1":0}
...
for epoch in range(N_EPOCHS):
    ...
    if best_result["valid_f1"] < valid_f1:
        best_result["epoch"] = epoch
        best_result["train_acc"] = train_acc
        best_result["test_f1"] = valid_acc
        best_result["test_acc"] = test_acc
    ...
    print(f'| Epoch: {epoch+1:02} | Train Loss: {train_loss:.3f} | Train Acc: {train_acc*100:.2f}% | Val. Loss: {valid_loss:.3f} | Val. Acc: {valid_acc*100:.2f}% | Test Loss: {test_loss:.3f} | Test Acc: {test_acc*100:.2f}% | Test F1: {test_f1*100:.2f}% |')
    #print ("TEST ACCURACY:", np.round(best_result["test_acc"] * 100, 2))
    print("TEST F1:",np.round(best_result["test_f1"] * 100, 2))
