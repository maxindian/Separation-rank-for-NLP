...
class WSDModel(object):
	...
	learning_rate, lr_decay,batch_size, word, num_classes, forward_only=False):
		...
		with tf.variable_scope("idcnn"):
			...
			layerInput = tf.nn.conv2d(rnn_concatenated_state,
								filter_weights,
								strides=[1, 1, 1, 1],
								padding="SAME",
								name="init_layer")
		...
		with tf.variable_scope("output_projection"):
			W = tf.get_variable(
				"W",
				[2*self.num_filter, self.num_classes+1],
				initializer=tf.truncated_normal_initializer(stddev=0.1))
			b = tf.get_variable(
				"b",
				[self.num_classes+1],initializer = tf.constant_initializer(0.1))

			self.scores = tf.nn.xw_plus_b(conv[-1][0], W, b)
